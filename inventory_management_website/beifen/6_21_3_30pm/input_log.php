...
<div id="log_wrapper">
    <div id="log_show">
        <table id ="log_table">
            ...
            <tr><td><h3>Add new logs below:</h3></td><td><h3>Product</h3></td><td><h3>Brand</h3></td><td><h3>Quantity</h3></td><td><h3>Price</h3></td><td></td><td></td></tr>
...
function delete_user(r){
    ...
    var brand = r.parentNode.parentNode.parentNode.childNodes[2].childNodes[0].nodeValue;
    var quantity = r.parentNode.parentNode.parentNode.childNodes[3].childNodes[0].nodeValue;
    var price = r.parentNode.parentNode.parentNode.childNodes[4].childNodes[0].nodeValue;
    var key = r.parentNode.parentNode.parentNode.childNodes[5].childNodes[0].childNodes[0].nodeValue;
...
function show_form(output){
    ...
    for(var i =0; i<obj.logs.length; i++){
        ...
        cell8.innerHTML= "<span style ='display:none'>"+obj.logs[i].key+"</span>";  // obj.logs[i].key;
    }
    //document.getElementById("debug2").innerHTML= obj.logs[3].employee+"<br>"+obj.logs.length;
}
function ajax_trigger(sign, product, brand, quantity, price){
    //$("#debug").text(password);
    //alert(password);
    $.ajax({
        url: "input_table.php",
