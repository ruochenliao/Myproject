...
function insert_data($conn, $employee, $product, $brand, $quantity, $price){
        ...
        //$sql = "INSERT INTO input_table (employee, product, brand, quantity, price) VALUES ('','$product','$brand','$quantity','$price')";
}
function add($conn){
...
function read($conn){
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            $row["AddDate"] = toTimeZone($row["AddDate"], $from_tz = 'America/New_York', $to_tz = 'America/Los_Angeles', $fm = 'Y-m-d H:i:s');
            $read_from_table .='{"product":"'.$row["product"].'", "brand":"'.$row["brand"].'", "quantity":"'.$row["quantity"].'", "price":"'.$row["price"].'", "AddDate":"'.$row["AddDate"].'", "key":"'.$row["id"].'","page":"'.$_GET["page"].'"},';
        }
