...
function insert_data($conn, $employee, $product, $brand, $quantity, $price){
        ...
        $sql = "INSERT INTO consume_table (employee, product, brand, quantity, price,Admin) VALUES ('$employee','$product','$brand','$quantity','$price','$Admin')";
        if ($conn->query($sql) === TRUE) {
            $return_pair = '{"logs":[{"employee":"'.$employee.'", "product":"'.$product.'", "brand":"'.$brand.'", "quantity":"'.$quantity.'", "price":"'.$price.'","key":"'.$conn->insert_id.'"'; 
            $sql = "SELECT AddDate FROM consume_table WHERE id='$conn->insert_id'";            
...
function check_duplicate($conn, $employee, $product, $brand, $quantity, $price){
    $sql ="SELECT * FROM consume_table WHERE employee='$employee' AND product ='$product'AND brand ='$brand'AND quantity ='$quantity'AND price ='$price'";
    $result= $conn -> query($sql);
...
function read($conn){
    $sql = "SELECT id, employee, product, brand, quantity, price, AddDate FROM consume_table ORDER BY AddDate DESC";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $read_from_table ='{"logs":[';
        while($row = $result->fetch_assoc()) {
            $row["AddDate"] = toTimeZone($row["AddDate"], $from_tz = 'America/New_York', $to_tz = 'America/Los_Angeles', $fm = 'Y-m-d H:i:s');
            $read_from_table .='{"employee":"'.$row["employee"].'", "product":"'.$row["product"].'", "brand":"'.$row["brand"].'", "quantity":"'.$row["quantity"].'", "price":"'.$row["price"].'","AddDate":"'.$row["AddDate"].'","key":"'.$row["id"].'","page":"'.$_GET["page"].'"},';
        }
...
function update($conn){
    ...
    $key= (int)$_GET["key"];
    if( strcmp($_SESSION["id"],"admin" ) ==0 ){
        $sql = "UPDATE consume_table SET employee='$employee', product='$product', brand='$brand', quantity='$quantity', price='$price' WHERE id ='$key'";
    }
    else if ( strcmp($_SESSION["id"],"client")==0  ){
        $Admin = "client".$_SESSION["realname"].$_SESSION["username"];
        $sql = "UPDATE consume_table SET product='$product', brand='$brand', quantity='$quantity', price='$price' WHERE id ='$key' AND Admin='$Admin'";
    }
    else{
...
function delete($conn){
    ...
    $key =(int)$_GET["key"];
    if( strcmp($_SESSION["id"],"admin" ) ==0 ){
        $sql = "DELETE FROM consume_table WHERE product='$product' AND brand='$brand' AND quantity='$quantity' AND price='$price' AND id ='$key'";
        echo "admin: ".$sql;
    }
    else if ( strcmp($_SESSION["id"],"client")==0  ){
        $Admin = "client".$_SESSION["realname"].$_SESSION["username"];
        $sql = "DELETE FROM consume_table WHERE employee='$employee' AND product='$product' AND brand='$brand' AND quantity='$quantity' AND price='$price' AND id ='$key' AND Admin='$Admin'"; 
        echo "client: ".$sql;
    }
    else{
...
if( strcmp($_GET["purpose"],"logout")==0 ){
