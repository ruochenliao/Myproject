...
function insert_data($conn,$realname,$user,$pass){
    if( check_duplicate($conn,$realname,$user,$pass)==true )
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            $return_pair = '{"logs":[{"realname":"'.$realname.'", "user":"'.$user.'","pass":"'.$pass.'", "key":"'.$conn->insert_id.'"}]}';
            echo $return_pair;
        }
        else {
...
function read($conn){
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            $read_from_table .= '{"realname":"'.$row["realname"].'","user":"'.$row["user"].'","pass":"'.$row["pass"].'","key":"'.$row["id"].'"},';
        }
...
function update($conn){
    ...
    if( check_duplicate($conn,$realname,$user,$pass)===true )
        ...
        $sql = "UPDATE id_table SET realname='$realname', user='$user', pass='$pass' WHERE id='$key'";
        //echo $sql;
        if( $conn->query($sql) ===TRUE)
            echo "update";
        else
