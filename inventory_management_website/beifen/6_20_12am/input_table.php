...
//    else{
    ...
    //$sql = "INSERT INTO input_table (employee, product, brand, quantity, price,Admin) VALUES ('$employee','$product','$brand','$quantity','$price','$Admin');"."SELECT LAST_INSERT_ID();";
    //$sql = "INSERT INTO input_table (employee, product, brand, quantity, price,Admin) VALUES ('$employee','$product','$brand','$quantity','$price','$Admin');# Affected rows: 1"." # Affected rows: 1"." SELECT LAST_INSERT_ID();";
    //$sql = 'INSERT INTO input_table (employee, product, brand, quantity, price,Admin) VALUES ('$employee','$product','$brand','$quantity','$price','$Admin');# Affected rows: 1'.' # Affected rows: 1'.' SELECT LAST_INSERT_ID();';
        if ($conn->query($sql) === TRUE) {
            //printf("id is :%d\n",$conn->insert_id);
            //echo $conn->insert_id();
...
function read($conn){
    $sql = "SELECT id, employee, product, brand, quantity, price, AddDate FROM input_table ORDER BY employee DESC";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $read_from_table ='{"logs":[';
        while($row = $result->fetch_assoc()) {
            $read_from_table .='{"employee":"'.$row["employee"].'", "product":"'.$row["product"].'", "brand":"'.$row["brand"].'", "quantity":"'.$row["quantity"].'", "price":"'.$row["price"].'"},';
        }
